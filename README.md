...
├── data/
│   ├── repositories/
│   │   └── locale_repository.dart    # Delega load/save ao LocaleService
│   └── services/
...
### 6.4 `LocaleRepository`: camada de acesso a dados

O `LocaleRepository` pertence à camada de Dados e apenas delega operações de leitura e escrita ao `LocaleService`, sem armazenar estado próprio:
...
main()
  ...
  │     └─ delega load/save ao LocaleService
  │
  ├─ ServiceLocator.instance.setup(repository: repository)
...
Usuário seleciona "Português" no dropdown
  │
  ├─ AppDropdownMenu.onSelected("Português")
